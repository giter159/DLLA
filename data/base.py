...
def get_data(args, logger):
    ...
    text_keyword = get_t_data(args, data_args,key = 'keyword')
    text_keyframe = get_t_data(args, data_args,key='keyframe')
    ...
    mm_train_data = MMDataset(train_label_ids, text_data['train'],text_keyword['train'] ,text_keyframe['train'],video_data['train'], audio_data['train'])
    mm_test_data = MMDataset(test_label_ids, text_data['test'],text_keyword['test'],text_keyframe['test'],video_data['test'], audio_data['test'])

    mm_data = {'train': mm_train_data, 'test': mm_test_data}
    
    train_outputs = {
        'text': text_data['train'],
        'text_keyword': text_keyword['train'],
        'text_keyframe': text_keyframe['train'],
