...
from .pretrain import PretrainDLLAManager
...
class DLLAManager:
    ...
    def __init__(self, args, data, model):
        ...
        pretrain_manager = PretrainDLLAManager(args, data, model)
    ...
    def _train(self, args): 
        ...
        for epoch in trange(int(args.num_train_epochs), desc='Epoch'):
            ...
            for batch_sup in tqdm(pseudo_sup_train_dataloader, desc = 'Iteration'):
                ...
                text_keyword = batch_sup['text_keyword'].to(self.device)
                text_keyframe = batch_sup['text_keyframe'].to(self.device)
                ...
                with torch.set_grad_enabled(True):
                    ...
                    _, mlp_output_a = self.model(text_feats, text_keyword ,text_keyframe ,torch.zeros_like(video_feats).to(self.device), audio_feats, mode='train-mm')
                    _, mlp_output_b = self.model(text_feats, text_keyword ,text_keyframe ,video_feats, torch.zeros_like(audio_feats).to(self.device), mode='train-mm')
                    _, mlp_output_c = self.model(text_feats, text_keyword ,text_keyframe ,video_feats, audio_feats, mode='train-mm')
            ...
            if len(non_select_ids) != 0:
                ...
                for batch_unsup in tqdm(pseudo_unsup_train_dataloader, desc = 'Iteration'):
                    ...
                    unsup_text_keyword = batch_unsup['text_keyword'].to(self.device)
                    unsup_text_keyframe = batch_unsup['text_keyframe'].to(self.device)
                    unsup_video_feats = batch_unsup['video_feats'].to(self.device)
                    unsup_audio_feats = batch_unsup['audio_feats'].to(self.device)

                    with torch.set_grad_enabled(True):
    
                        _, mlp_output_a = self.model(unsup_text_feats,unsup_text_keyword, unsup_text_keyframe,torch.zeros_like(unsup_video_feats).to(self.device), unsup_audio_feats, mode='train-mm')
                        _, mlp_output_b = self.model(unsup_text_feats, unsup_text_keyword, unsup_text_keyframe,unsup_video_feats, torch.zeros_like(unsup_audio_feats).to(self.device), mode='train-mm')
                        _, mlp_output_c = self.model(unsup_text_feats, unsup_text_keyword, unsup_text_keyframe,unsup_video_feats, unsup_audio_feats, mode='train-mm')
    ...
    def _get_outputs(self, args, mode, return_feats = False, modality = 'tva'):
        ...
        for batch in tqdm(dataloader, desc="Get Outputs"):
            ...
            text_keyword = batch['text_keyword'].to(self.device)
            text_keyframe = batch['text_keyframe'].to(self.device)
            ...
            with torch.set_grad_enabled(False):
                if modality == 'tva':
                    features, _ = self.model(text_feats, text_keyword ,text_keyframe ,video_feats, audio_feats, mode='train-mm')
                elif modality == 't0a':
                    features, _ = self.model(text_feats, text_keyword ,text_keyframe ,torch.zeros_like(video_feats).to(self.device), audio_feats, mode='train-mm')
                elif modality == 'tv0':
                    features, _ = self.model(text_feats, text_keyword ,text_keyframe ,video_feats, torch.zeros_like(audio_feats).to(self.device), mode='train-mm')
    ...
    def load_pretrained_model(self, pretrained_model):
        ...
        # "method_model.cluster_centers", "method_model.contrast_head.0.weight", "method_model.contrast_head.0.bias", "method_model.contrast_head.2.weight", "method_model.contrast_head.2.bias"
