    ...
    def _train(self, args):
        ...
        for epoch in trange(int(args.num_pretrain_epochs), desc="Epoch"):
            ...
            for batch in tqdm(pseudo_dataloader, desc = "Iteration"):
                ...
                # print("Tensor device:", text_feats.device)
                
                with torch.set_grad_enabled(True):

                    mlp_output_a = self.model(text_feats,text_keyword ,text_keyframe ,torch.zeros_like(video_feats).to(self.device), audio_feats, mode='pretrain-mm')
                    mlp_output_b = self.model(text_feats, text_keyword ,text_keyframe ,video_feats, torch.zeros_like(audio_feats).to(self.device), mode='pretrain-mm')
                    mlp_output_c = self.model(text_feats, text_keyword ,text_keyframe ,video_feats, audio_feats, mode='pretrain-mm')
