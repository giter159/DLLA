...
def get_pseudo_dataloader(args, train_outputs, mode='pretrain', pseudo_labels=None):
    ...
    text_keyword = train_outputs['text_keyword']
    text_keyframe = train_outputs['text_keyframe']
    ...
    if 'select_ids' in train_outputs.keys():
        ...
        text_keyword = [text_keyword[i] for i in select_ids]
        text_keyframe = [text_keyframe[i] for i in select_ids]
        new_video = {}
        ...
        train_data = MMDataset(train_label_ids, text, text_keyword, text_keyframe, new_video, new_audio)

    else:
        train_label_ids = torch.tensor(pseudo_labels).unsqueeze(1)
        train_data = MMDataset(train_label_ids, text, text_keyword, text_keyframe, video, audio)
